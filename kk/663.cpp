...
void build_tree(ll root,ll st,ll ed){
	...
	segt[root]=segt[root*2+1]+segt[root*2+2];
}
...
ll query_tree(ll root,ll nst,ll ned,ll qst,ll qed){
	...
	return query_tree(root*2+1,nst,mid,qst,qed)+query_tree(root*2+2,mid+1,ned,qst,qed);
}
int main(){
	scanf("%lld %lld",&n,&m);
	build_tree(0,1,n);
	for(ll i=1;i<=m;i++){
		...
		else ans[cnt++]=query_tree(0,1,n,x,y)%mod;
	}
